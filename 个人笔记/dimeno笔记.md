...
![image-20230925134808323](https://raw.githubusercontent.com/Rainchen0504/picture/master/202309251348229.png)

1. 每个项目中包含了多个页面，项目对象包含页面列表数组，页面列表数组维护了整个项目的不同页面，每个页面对象中又维护了组件列表，包含了我们配置的不同的组件对象实例，最后被components动态组建渲染到页面上展示；
2. 可视化平台由以下五部分组成：左侧菜单，右侧配置列表，顶部工具栏，底部缩放显示和中间画布
   1. 左侧菜单：包含三个tab，图层列表，组件库和页面列表。
      1. 图层用列表的形式展示了当前页面已添加的所有组件，在此列表中可以控制组件的显示、隐藏、删除、编组、层级等；
      2. 组件库包含目前已开发的57个可选择组件，包含图表、地图、媒体、文字等；
      3. 页面列表则展示了当前项目的所有页面列表，可以切换配置不同的页面；
   2. 右侧配置列表
      1. 组件未选中状态时用来控制画布的背景、大小、缩放比例等；
      2. 组件选中状态时一般包含三个tab
         1. 样式配置，控制组件的可配置属性，如文本属性、颜色属性、排列方式等；
         2. 数据配置，配置组件接受的数据源，可接受静态数据或请求资源，可以编写过滤器得到符合要求的数据结果；
         3. 交互配置，配置组件的交互能力，如通过点击或切换实现页面的切换和动画效果等；
   3. 顶部工具栏
      1. 用来管理全局自定义参数、过滤器、回收站和预览发布功能
   4. 底部缩放显示
      1. 控制画布部分的缩放和版本号显示；
   5. 中间画布
      1. 核心部分，展示配置好的组件实例和页面效果；
3. 组件的创建：选中组件库中的组件，系统会为该组件创建一个组件对象，该对象包含默认长、宽、位置、类型和data、attr属性，data属性保存组件的数据对象，attr属性保存组件的样式和交互属性。然后在Vuex中发起请求保存组件对象，创建成功后执行两件事，将创建的组件对象保存到Vuex中维护的comStore中，同时默认选中创建的组件。
4. comStore：comStore中最核心的工作是维护组件字典comDic和当前选中组件comData，组件字典维护了当前页面的组件列表，当前选中组件则存储了当前正在选中的组件属性。
