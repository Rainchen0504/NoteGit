...
​	不会创建自身的`this`，只会从作用域链的上一次继承`this`。箭头函数this在定义时候就已经确认了，由词法作用域决定，不会被调用对象改变。
...
箭头函数没有自己的`this`和`arguments`，不能作为构造函数被`new`操作符调用。



## 18、call、apply和bind

| 方法                              | 作用         | 参数                                           | 返回结果         | 执行时机     | 写法                             |
| --------------------------------- | ------------ | ---------------------------------------------- | ---------------- | ------------ | -------------------------------- |
| <font color=pink>**call**</font>  | 改变this指向 | 第一个参数是要绑定this的对象和其他**参数序列** |                  | 立即执行函数 | `a.call({}, arg1, arg2)`         |
| <font color=pink>**apply**</font> | 改变this指向 | 第一个参数是要绑定this的对象和其他**参数数组** |                  | 立即执行函数 | `a.call({}, [arg1, arg2])`       |
| <font color=pink>**bind**</font>  | 改变this指向 | 第一个参数是要绑定this的对象和其他**参数序列** | this改变后的函数 | 不调用函数   | `var r = a.bind({}, arg1, arg2)` |
...
1. call和apply会调用函数并改变函数内部的this指向；
2. bind不会调用函数但是可以改变函数内部的this指向；
3. call接收参数的形式是参数序列，而apply接收参数的形式是数组；
