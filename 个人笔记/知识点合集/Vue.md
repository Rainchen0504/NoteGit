...
1. 识别每个元素：key属性是用来区分for循环中每个元素的唯一标识，使用key可以跟踪每个节点的身份，方便在数据发生改变时，能够高效的更新和复用现有的元素，而不是重新创建和销毁。如果没有提供key，Vue会发出警告，并频繁的更新替换元素，使整个patch的过程比较低效，影响性能。
2. 高效的的更新策略：Vue使用key属性确定哪些项目是已经存在的，哪些是需要更新的，哪些是需要销毁的、哪些是需要新添加的。通过比较新旧节点的key值，Vue可以高效的知道元素要执行的操作。如果没有提供key，就只能使用依次替换的低效更新方式，导致不必要的性能消耗。
3. 维持组件状态：如果使用`v-for`循环渲染的是组件，那么每个组件实例都需要有一个唯一的`key`。这样，当数组发生变化时，Vue.js可以正确地跟踪每个组件实例的状态，并保留组件的正确状态，而不是将状态混淆或重新排序。如果没有提供`key`，组件状态可能会出现错误，导致不一致的渲染结果或意外的行为。
