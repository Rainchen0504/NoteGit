...
Vue是组件级更新，如果不采用异步更新，那么每次更新数据都会对当前组件进行重新渲染，所以为了性能，Vue会在本轮数据更新后，再异步更新视图。

`dep.notify()`通知订阅者`watcher`进行更新并依次调用`watcher`的update方法，任务队列整理并去重更新任务，使用`nextTcik`在下一轮事件中刷新订阅者`watcher`队列，实现异步更新。



## 21、$nextTick原理和作用

​	由于Vue更新DOM是异步执行的，当数据发生变化，Vue会开启一个异步更新队列，视图需要等队列中数据发生变化完成之后再统一进行更新。而nextTick会在在下一次DOM更新循环结束之后执行延迟回调。这样就可以获取到最新的数据和DOM。



### （1）可以获取到最新DOM的原因

​	由于Vue.js采用了异步更新队列来进行更新DOM。因为Vue使用虚拟DOM进行渲染，变化侦测的通知只发送到组件，组件内用到的所有状态都会通知到同一个watcher，然后虚拟DOM会对整个组件进行对比并更新DOM。

​	也就是说，如果在同一轮事件循环中有两个数据发生了变化，那么组件的watcher会受到两份通知，从而进行两次渲染。事实上，并不需要渲染两次，虚拟DOM会对整个组件进行渲染，所以只需要等所有状态都修改完毕后，一次性将整个组件的DOM渲染到最新即可。

​	要解决整个问题，Vue.js的实现方式是将受到通知的watcher实例添加到队列中缓存起来，并且在添加到队列之前检查其中是否已经存在相同的watcher，只有不存在时，才将watcher实例添加到队列中。然后在下一次事件循环中，Vue.js会让队列中的watcher触发渲染流程并清空队列。这样就可以即便在同一事件中有两个状态发生改变，watcher最后也执行一次渲染流程。`下次DOM更新周期`的意思是`下次微任务执行时更新DOM`，而`vm.$nextTick`其实是将回调添加到`微任务中（只有在特殊情况下才降级到宏任务）`，所以我们要在$nextTick中才能拿到数据改变后的最新DOM。
...
- Vue刚开始时候，`$nextTick`是宏任务`setTimeout`，但是这种方法性能太差；
- 后来使用微任务`Mutation Observer`，但是更新速度太快，且兼容性不好，很多浏览器不支持；
- 之后更新成微任务和宏任务并行：先判断是否支持`Mutation Observer`，支持就使用，不支持就使用宏任务；
- Vue2.5版本以后，修复了微任务问题。最新的`$nextTick`采用的都是纯微任务；



### （3）应用场景

- 数据变化后执行某个操作，这个操作需要使用随数据变化而变化的DOM结构；
- 如果生命周中，在creaeted钩子进行DOM操作，也一定要放在`nextTick`回调函数中；
...
- update：所在组件的VNode更新时调用，但是可能发生在其子VNode更新之前调用；
- ComponentUpdate：指令所在组件的 VNode及其子VNode全部更新后调用；
- unbind：只调用一次，指令与元素解绑时调用；

#### 2、Vue3

- created：在绑定元素的attribute前
- beforeMount：元素被插入到DOM前调用
- mounted：绑定元素挂载完成后
- beforeUpdate：更新前
- updated：绑定元素更新后
- beforeUnmount：卸载前
...
1. el：指定绑定的元素，可以直接操作DOM；
2. binding：一个对象，包含属性：
   1. value：传递给指令的值；
   2. oldValue：绑定的前一个值；
   3. arg：传给指令的参数；
   4. instance：使用该指令的组件实例；
   5. modifiers：一个包含修饰符的对象；
3. vnode：编译生成的虚拟节点；
4. prevNode：之前的渲染中指令所绑定元素的虚拟节点；



## 23、mixin、mixins、extend、extends

`mixins` 和 `extends`均是用于合并、拓展组件的，两者**均通过 `mergeOptions` 方法**实现合并。
...
​	接收一个混入对象的数组，其中混入对象可以像正常的实例对象一样包含实例选项，这些选项会被合并到最终的选项中。

1. 如果是data中返回的对象的属性相同，则以组件的值为准；
2. 如果是生命周期钩子函数，则都生效；
3. methods、components、directive对象等会合并成一个对象，方法名不同时都会执行；方法名相同时只会执行当前组件的方法；
...
​	对象是引用类型，当复用组件时，由于数据对象都指向同一个data对象，在一个组件中修改data，其他组件中的data会被同时修改。

​	而使用返回对象的函数时，由于每次返回的都是一个新对象，引用地址不同，不会产生影响干涉的问题。
...
Vue2常见问题就是操作数据更新了，但是视图没有更新；



#### 2、原因

1. `vue2`是用过`Object.defineProperty`实现数据响应式，当访问定义的属性或者修改属性值的时候都能够触发`setter`与`getter`；
2. 但是为对象添加新属性时候，无法通过`Object.defineProperty`设置成响应式数据，因此无法触发属性拦截；



#### 3、解决方法

1. `Vue.set()`方法---为对象添加少量新属性

   - 通过`Vue.set`向响应式对象中添加一个`property`，并确保这个新 `property`同样是响应式的，且触发视图更新

2. `Object.assign()`方法---为对象添加大量新属性

   - 直接使用`Object.assign()`添加到对象的新属性不会触发更新

   - 应创建一个新的对象，合并原对象和混入对象的属性

3. `$forceUpdate`方法---不建议使用

   - `$forceUpdate强迫`Vue` 实例重新渲染，仅仅影响实例本身和插入插槽内容的子组件，而不是所有子组件



### （2）Vue3

Vue3通过`Proxy`实现数据响应式的，直接动态添加新属性仍可以**实现数据响应式**。



## 26、Vue监听对象数组变化

​	Vue中某个属性发生变化时，不会立即同步执行DOM重新渲染，而是按照一定的策略进行DOM更新。会采用异步更新的方法，只要侦听到数据变化， Vue 将开启一个队列，并缓冲在同一事件循环中发生的所有数据变更。

​	如果同一个订阅者watcher被多次触发，只会被推入到队列中一次，这种优化对于避免不必要的计算和 DOM 操作是非常重要的。然后在下一个的事件循环tick中，Vue 刷新队列并执行实际（已去重的）工作。

```js
// 调用set方法
this.$set(要改变的数组/对象， 要改变的位置/key， 修改的目标值value)
// 调用数组方法
splice()、 push()、pop()、shift()、unshift()、sort()、reverse() 
```

vue源码里缓存了array的原型链，然后重写了这几个方法，触发这几个方法的时候会observer数据，意思是使用这些方法不用我们再进行额外的操作，视图自动进行更新。



## 15、Vue.$set原理

- 如果目标是数组，直接用数组的splice方法触发响应式；
- 如果目标是对象，
